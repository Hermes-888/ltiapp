...
class BLTI {
    ...
    function __construct($parm=false, $usesession=true, $doredirect=true) {
        ...
        // look it up in a database from parameters given
        $secret = false;
        $row = false;
        if ( is_string($parm) ) {
            $secret = $parm;
		...
		// pdo version
			try {
				$conn = new PDO('mysql:dbname=aviationmedia;host=mediafiles.uvu.edu', 'mediaadmin', '+8?hooraY3');
				$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				
				$stmt = $conn->prepare('SELECT * FROM '.$parm['table'].' WHERE key_column = :key');
				$stmt->bindValue(':key', $oauth_consumer_key, PDO::PARAM_STR);
				$stmt->execute();
			 //NOTE SENT: $consumer= ['table'=>'consumers','key_column'=>$consumer_key];// $parm['key_column'] == $oauth_consumer_key
				$result = $stmt->fetchAll();
				
				if ( $stmt->rowCount() != 1 ) {
					$this->message = "Your consumer is not authorized oauth_consumer_key=".$oauth_consumer_key." R=".$result;
					return;
				} else {
					foreach($result as $row) { 
						$secret = $row['secret_column'];
						$context_id = $row['context_column'];
						if ( $context_id ) $this->context_id = $context_id;
						$this->row = $row;
						break;
					}
					if ( ! is_string($secret) ) {
						$this->message = "Could not retrieve secret oauth_consumer_key=".$oauth_consumer_key." S=".$secret;
						return;
					}  
				}
...
// add getSecret ??? for student view grade media ???
    function dump() { 
